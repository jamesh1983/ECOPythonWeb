    ...
    try:
        ...
        if (len(data)>=1):
            for row in data:
                ...
                message['warningcode'] = row[0]#故障码
                message['SN'] = row[1]#模块
                message['equipment'] = row[2]#设备
                message['warning_setting'] = str(row[3])#报警设置
                message['warning_status'] = "报警已持续" + row[4] + "分钟,未被确认"#报警状态
                if(row[5] != None):
                    userID_List = row[5].split(',')#报警用户
                else:
                    userID_List = ""
    ...
    try:
        ...
        if (len(data)>=1):
            for row in data:
                ...
                message['warningcode'] = str(row[0])#故障码
                message['SN'] = str(row[1])#模块
                message['equipment'] = str(row[2])#设备
                message['warning_setting'] = str(row[3])#报警设置
                message['warning_status'] = "报警已持续" + str(row[4]) + "分钟，已被" + str(row[6]) + "确认"#报警状态
                if(row[5] != None):
                    userID_List = row[5].split(',')#报警用户
                else:
                    userID_List = ""
